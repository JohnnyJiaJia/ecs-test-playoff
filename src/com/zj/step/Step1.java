...
public class Step1 {
	...
	public static VMsolu predictByAvg(int[][] data, Info info) {
		int num = data[0].length - info.getPredictDays();
		//VM规格数
		int numFlavor = data.length;
		VMsolu vmsolu = new VMsolu(numFlavor);
		...
		for (int i = 0; i < numFlavor; i++) {
			...
			BigDecimal b = new  BigDecimal(avg[i] * info.getPredictDays() * 2);
	...
	public static VMsolu presictByESMperDay(int[][] data, Info info) {
		...
		double[][] spaceData = EMSsolver.run(data, info, numSpaceDays, 0.6);
		double[][] newData = addMartix(data, spaceData);
		double[][] result = EMSsolver.run(newData, info, numPredictDays, 0.7);
		VMsolu vmsolu = new VMsolu(numFlavor);
		for (int i = 0; i < numFlavor; i++) {
			double sum = 0;
